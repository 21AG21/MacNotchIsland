...
struct GeneralPane: View {
    ...
    private var hidingFooter: String {
        Self.hidingFooter(needsAccessibility: needsAccessibilityForFullScreen)
    }

    /// The Hiding section's footer, with or without the note on Accessibility. It says where
    /// the switch starts, because that is not the same on every Mac: until it is set it
